...
router.get('/', async (ctx: Koa.Context) => {
  const token = jwt.sign(userInfo, 'ctx.env.accessSecretKey', { expiresIn: '1h' }); //ctx.env.accessSecretKey
...
router.get('/test', async (ctx: Koa.Context) => {
  ...
  const user = jwt.verify(token, 'ctx.env.accessSecretKey'); // ctx.env.accessSecretKey
  ctx.body = user;
