...
const { answerPostAnswersBody, answerPostScoringBody } = require('../type');
...
router.post('/:assignmentId/:userNumber', async (ctx: Koa.Context) => {
  ...
  body.answers.forEach((element: typeof answerPostScoringBody) => {
  ...
  for (let a = 0; a < studentAnswerPaper.answers.length; a += 1) {
    const s = body.answers
      .find((element: typeof answerPostScoringBody) => element
        .questionId === studentAnswerPaper.answers[a].questionId);
