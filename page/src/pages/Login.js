...
function Login(props){
	...
	async function hashProcess(){
		try {
			let hashedToken = await axios.get('/v1/token', { withCredentials: true });

			if (hashedToken.status === 404) {
				alert("내부 서버 오류로 token을 찾을 수 없습니다. 로그인을 다시 시도해주세요.");
				return;
			}
			// hash password
			let hashedPw = hashData(hashData(pw));
			// hash result
			let result = hashData(hashedToken.data + hashedPw);
	...
	async function setLoginData(e) { // pw 암호화 및 api data 받기
		setButtonText("로그인 중...");
		let hashedPw = await hashProcess();
	...
	async function loginAsTestProfessorCookie(e) {
		...
		let hashedToken = await axios.get(
